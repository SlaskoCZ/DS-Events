<template>
  ...
  >
    <p 
      v-if="text != ''"
      class="group-name h1-small-small h1-small-medium padding-top-24 margin-0 padding-left-16 padding-left-26-medium padding-right-16"
      :class="textClass"
    >
      <strong>{{ text }}</strong> 
      <svg 
    ...
    <div class="padding-first-top padding-last-bottom">
      <template v-for="message in view">
        <EventMessage
          ...
          :class="{'color-gray': !message.isNew, 'color-dark-gray': message.isNew}"
        />
        <EventGroupMessage
          v-if="message.subMessages != undefined && message.subMessages.length > 1"
          :key="message.index"
